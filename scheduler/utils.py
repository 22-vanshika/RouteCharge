...
def cumulative_km_to(route: Route, direction: Direction, target_id: str, context = None) -> float:
    ...
    if context and hasattr(context, "station_distances"):
        if (direction, target_id) in context.station_distances:
            return context.station_distances[(direction, target_id)]
...
def expected_arrival(scenario: Scenario, candidate: ChargingCandidate, context = None) -> int:
    ...
    if context and hasattr(context, "bus_by_id"):
        bus = context.bus_by_id[candidate.bus_id]
    else:
        bus = next(b for b in scenario.buses if b.id == candidate.bus_id)

    target_km = cumulative_km_to(scenario.route, bus.direction, candidate.station_id, context)
    ...
    if prior and prior.charging_stops:
        ...
        last_km = cumulative_km_to(scenario.route, bus.direction, last.station_id, context)
